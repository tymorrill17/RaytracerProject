...
#include "stb-master/stb_image_warnings.h"
#include "perlin.h"
#include <iostream>
...
class checker_texture : public texture {
	public:
		checker_texture() {}
		checker_texture(shared_ptr<texture> ev, shared_ptr<texture> od) : even(ev), odd(od) {}
...
class image_texture : public texture {
	public:
		...
		virtual color value(double u, double v, const vec3& p) const override {
			// If we have no texture data, return cyan as debugging aid:
			if (data == nullptr)
				return color(0, 1, 1);
			...
			// Clamp integer mapping, since actual coords should be less than 1.0

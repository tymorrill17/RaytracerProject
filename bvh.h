...
inline bool box_compare(const shared_ptr<hittable> a, const shared_ptr<hittable> b, int axis) {
	...
	if(!a->bounding_box(0, 0, box_a) || !b->bounding_box(0, 0, box_b))
		std::cerr << "In \"bvh.h\", Line 33: No bounding box in bvh_node constructor.\n";
...
bvh_node::bvh_node(const std::vector<shared_ptr<hittable>>& src_objects, size_t start, size_t end, double time0, double time1) {
	auto objects = src_objects; // Modifiable array of source scene objects.
	...
	if (!left->bounding_box(time0, time1, box_left) || !right->bounding_box(time0, time1, box_right))
		std::cerr << "In \"bvh.h\", Line 93: No bounding box in bvh_node constructor. \n";
